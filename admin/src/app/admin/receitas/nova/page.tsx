...
import { ChevronsUpDown, PlusCircle, Trash2, BookOpen, Video, Info } from 'lucide-react';
...
export default function NovaReceita() {
  ...
  const loadFreeCount = async () => {
    try {
      const data = await api.getReceitas({ incluirInativas: true });
      if (data && Array.isArray(data)) {
        const freeRecipes = data.filter((r: any) => r.is_free && r.ativa);
        setFreeCount(freeRecipes.length);
      }
    } catch (err) {
      console.error('Erro ao carregar contagem de receitas FREE:', err);
    }
  };
  ...
  const validate = (): boolean => {
    ...
    if (!formData.tempo_preparo || parseInt(formData.tempo_preparo) <= 0) newErrors.tempo_preparo = 'Tempo inválido';
    setErrors(newErrors);
