    ...
    <div className="p-6 sm:p-10 bg-[#fafafa] dark:bg-[#0a0a0a] min-h-screen pb-24">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-gray-100 dark:border-[#222] pb-8 gap-6">
          <div className="flex items-center gap-4">
            <div>
              <h1 className="text-xl font-light text-gray-400 dark:text-gray-500 tracking-tight uppercase">
                Biblioteca <span className="text-gray-800 dark:text-white font-normal">Mídias</span>
              </h1>
              <p className="text-[10px] text-gray-400 font-normal uppercase tracking-[0.2em] mt-1">Armazenamento Cloud Privado</p>
            </div>
            <div className="bg-white dark:bg-[#111] px-3 py-1.5 rounded-lg border border-[#c8921a]/20 shadow-sm ml-2 flex items-center gap-2">
              <span className="text-xs font-normal text-[#c8921a] tracking-tight">{totalItems}</span>
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              className="px-4 py-1.5 rounded-md border border-[#c8921a]/60 text-[#c8921a] text-[9px] font-normal uppercase tracking-widest hover:bg-[#c8921a]/5 hover:border-[#c8921a] transition-all duration-300 flex items-center gap-1.5"
            >
              <FolderPlus size={12} /> Categoria
            </button>
            <button 
              onClick={() => setIsUploadModalOpen(true)}
              className="px-4 py-1.5 rounded-md border border-[#c8921a] text-[#c8921a] text-[9px] font-normal uppercase tracking-widest hover:bg-[#c8921a] hover:text-white transition-all duration-300 flex items-center gap-1.5"
            >
            ...
            exercicios.map((ex) => (
              <div key={ex.id} className="group bg-white dark:bg-[#111] border border-[#c8921a]/30 dark:border-[#c8921a]/20 rounded-xl overflow-hidden hover:border-[#c8921a]/60 hover:shadow-sm transition-all duration-300 flex flex-col">
                <div onClick={() => { setPreviewVideo(ex); setVideoError(false); setVideoIsLoading(true); }} className="w-full h-[160px] bg-[#000] relative cursor-pointer overflow-hidden flex items-center justify-center">
                  ...
                  <div className="absolute top-3 left-3 z-20 pointer-events-none"><span className="bg-[#c8921a] text-white text-[7px] px-2 py-0.5 rounded font-black uppercase tracking-[0.2em] shadow-sm">{ex.categoria || 'Geral'}</span></div>
                </div>
            ...
            <div className="relative flex-1 bg-black flex flex-col items-center justify-center overflow-hidden min-h-[300px]">
              ...
              {videoError ? (
                <div className="text-center p-10 z-10">
                  ...
                  <h3 className="text-white font-normal text-xs mb-6 uppercase tracking-widest">Streaming indisponível</h3>
        ...
        <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in zoom-in duration-200">
          <div className="bg-white dark:bg-[#111] rounded-2xl shadow-xl w-full max-w-xs overflow-hidden border border-gray-100 dark:border-[#222]">
            <div className="px-5 py-3 border-b border-gray-100 dark:border-[#1a1a1a] flex items-center justify-between">
              <h2 className="text-[10px] font-normal uppercase tracking-widest text-gray-800 dark:text-white">Confirmar Exclusão</h2>
            ...
            <div className="p-6 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-[11px] leading-relaxed">Remover permanentemente <span className="text-gray-800 dark:text-white font-normal">"{itemToDelete.nome.toUpperCase()}"</span>?</p>
            </div>
      ...
      {isEditModalOpen && itemToEdit && (
        <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in zoom-in duration-200">
          <div className="bg-white dark:bg-[#111] rounded-2xl shadow-xl w-full max-w-xs overflow-hidden border border-gray-100 dark:border-[#222]">
            <div className="px-5 py-3 border-b border-gray-100 dark:border-[#1a1a1a] flex items-center justify-between">
              <h2 className="text-[10px] font-normal uppercase tracking-widest text-gray-800 dark:text-white">Editar Mídia</h2>
            ...
            <div className="p-6">
              ...
              <input type="text" value={editVideoExplicativoUrl} onChange={(e) => setEditVideoExplicativoUrl(e.target.value)} className="w-full bg-transparent border-t-0 border-l-0 border-r-0 border-b border-gray-300 dark:border-[#333] px-0 py-2 text-xs focus:outline-none focus:border-[#c8921a] text-gray-700 dark:text-white mb-2 font-normal" placeholder="https://..." />
              ...
              <div className="bg-gray-50 dark:bg-[#222] border border-gray-200 dark:border-[#333] rounded-lg p-2 max-h-32 overflow-y-auto custom-scrollbar mb-6">
                {categorias.map(cat => (
                  <label key={cat.id} className="flex items-center gap-2 p-2 hover:bg-white dark:hover:bg-[#333] rounded cursor-pointer transition-colors">
                    <input 
                      type="checkbox" 
                    ...
                    />
                    <span className="text-[10px] font-normal text-gray-700 dark:text-gray-300">{cat.nome}</span>
                  </label>
                ))}
                {categorias.length === 0 && (
              ...
              <button onClick={handleUpdateTitle} disabled={isSaving || !newTitle.trim()} className="w-full py-2.5 rounded-lg bg-[#c8921a] text-white text-[9px] font-black uppercase tracking-widest shadow-sm shadow-[#c8921a]/20 hover:bg-[#b07d14] transition-colors">Salvar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isCategoryModalOpen && (
        <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in zoom-in duration-200">
          <div className="bg-white dark:bg-[#111] rounded-2xl shadow-xl w-full max-w-xs overflow-hidden border border-gray-100 dark:border-[#222]">
            <div className="px-5 py-3 border-b border-gray-100 dark:border-[#1a1a1a] flex items-center justify-between">
              <h2 className="text-[10px] font-normal uppercase tracking-widest text-gray-800 dark:text-white">Gerenciar Categorias</h2>
            ...
            <div className="p-6">
              ...
              <div className="bg-gray-50 dark:bg-[#222] border border-gray-200 dark:border-[#333] rounded-lg p-2 max-h-40 overflow-y-auto custom-scrollbar mb-6">
                {categorias.map(cat => (
                  <div key={cat.id} className="flex items-center justify-between p-2 hover:bg-white dark:hover:bg-[#333] rounded transition-colors">
                    <span className="text-[10px] font-normal text-gray-700 dark:text-gray-300">{cat.nome}</span>
              ...
              <input type="text" value={newCategoryName} onChange={(e) => setNewCategoryName(e.target.value)} className="w-full bg-transparent border-t-0 border-l-0 border-r-0 border-b border-gray-300 dark:border-[#333] px-0 py-2 text-xs focus:outline-none focus:border-[#c8921a] text-gray-700 dark:text-white mb-6 font-normal" placeholder="Ex: Costas..." autoFocus />
              <button onClick={handleCreateCategory} disabled={isSaving || !newCategoryName.trim()} className="w-full py-2.5 rounded-lg bg-[#c8921a] text-white text-[9px] font-black uppercase tracking-widest shadow-sm shadow-[#c8921a]/20 hover:bg-[#b07d14] transition-colors">Criar</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white dark:bg-[#111] rounded-xl shadow-2xl w-full max-w-xl overflow-hidden border border-gray-100 dark:border-[#222]">
            <div className="px-8 py-4 border-b border-gray-50 dark:border-[#1a1a1a] flex items-center justify-between">
              <h2 className="text-[10px] font-normal uppercase tracking-widest text-gray-800 dark:text-white">Adicionar Mídia</h2>
      ...
      <button
        ...
        className="fixed bottom-8 right-8 z-[100] bg-[#c8921a] text-white p-3 rounded-full shadow-sm hover:bg-[#b07d14] transition-all duration-300 opacity-80 hover:opacity-100 group"
