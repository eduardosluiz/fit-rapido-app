...
export default function CategoriasTreinosPage() {
  const { isAuthenticated } = useAuth();
  const [categorias, setCategorias] = useState<CategoriaTreino[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.updateCategoriaTreino(editingId, formData);
      } else {
        await api.createCategoriaTreino(formData);
      }
      setShowForm(false);
      setEditingId(null);
  ...
  const handleEdit = (categoria: CategoriaTreino) => {
    setEditingId(categoria.id);
    setFormData({
      nome: categoria.nome,
      slug: categoria.slug,
      descricao: categoria.descricao || '',
      imagem_url: (categoria as any).imagem_url || '',
      ativa: categoria.ativa,
    });
    setShowForm(true);
    
    // Scroll para a categoria selecionada após um pequeno delay
    setTimeout(() => {
  ...
  const handleCreateDefault = async () => {
    const categoriasPadrao = [
      { nome: 'Treino de Força', slug: 'treino-de-forca', descricao: 'Treinos focados em ganho de massa muscular e força' },
      { nome: 'Cardio', slug: 'cardio', descricao: 'Exercícios cardiovasculares para melhorar condicionamento' },
      { nome: 'HIIT', slug: 'hiit', descricao: 'Treinos intervalados de alta intensidade' },
      { nome: 'Funcional', slug: 'funcional', descricao: 'Movimentos que simulam atividades do dia a dia' },
    ];

    try {
      for (const cat of categoriasPadrao) {
        await api.createCategoriaTreino({ ...cat, ativa: true });
      }
      await loadCategorias();
                ...
                >
                  ...
                  <FormField label="Slug" required fullWidth>
                    <Input
                      ...
                      placeholder="Ex: treino-de-forca"
                    />
                  </FormField>
                  ...
                  <FormField label="Descrição" fullWidth>
                    <Textarea
                      ...
                      rows={3}
                      placeholder="Descreva esta categoria de treino..."
