...
function EditarTreinoForm() {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const [treino, cats, mods, biblio] = await Promise.all([
        api.getTreino(id),
      ...
      setFormData({
        ...
        imagem_url: treino.imagem_url || '',
        video_url: treino.video_url || '',
        nivel: treino.nivel || 'iniciante',
        duracao_minutos: String(treino.duracao_minutos || ''),
        dias_por_semana: String(treino.dias_por_semana || ''),
        tipo_treino: treino.tipo_treino || 'academia',
        modalidade_id: treino.modalidade?.id || treino.modalidade_id || '',
        dia_semana: treino.dia_semana !== null && treino.dia_semana !== undefined ? String(treino.dia_semana) : '',
        categoria_ids: Array.isArray(treino.categorias) 
          ? treino.categorias.map((cat: any) => cat.id)
          : treino.categoria_id ? [treino.categoria_id] : [],
        grupos_musculares: Array.isArray(treino.grupos_musculares) ? treino.grupos_musculares.join(', ') : '',
        tags: Array.isArray(treino.tags) ? treino.tags.join(', ') : '',
        ativa: treino.ativa ?? true,
        is_premium: treino.is_premium || false,
  ...
  const validate = (): boolean => {
    ...
    if (formData.exercicios_detalhados.length === 0) newErrors.exercicios = 'Adicione ao menos um exercício';
    setErrors(newErrors);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.updateTreino(id, data);
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-[#1a1a1a] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 font-normal tracking-widest uppercase text-[10px]">Carregando Treino...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            ...
            <div>
              <h1 className="text-lg sm:text-xl font-light text-gray-600 dark:text-gray-400 tracking-tight uppercase">
                Editar <span className="text-gray-800 dark:text-white font-normal">Treino</span>
              </h1>
              <p className="text-[9px] sm:text-[10px] text-gray-500 dark:text-gray-400 font-normal uppercase tracking-[0.2em] mt-1">Atualização de Registro</p>
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <div className="lg:col-span-5 flex flex-col">
                  <div className="grid grid-cols-2 gap-4 flex-1">
                    <div className="space-y-2">
                      <label className="text-[9px] font-normal uppercase tracking-[0.15em] text-gray-800 dark:text-gray-200 ml-0.5">Identidade (Capa do Treino)</label>
            ...
            <div className="treino-edit-actions flex flex-col-reverse sm:flex-row gap-4 pt-10 border-t border-gray-200 dark:border-[#222] justify-end">
              ...
              >
                {saving ? (
                  ...
                  <>
                    <i className="bx bx-check text-lg"></i> Salvar Alterações
                  </>
                )}
              </button>
            </div>
